...
import com.aeye.mifss.common.exception.HSAFExceptionCode;
...
public class AeyeFSManager {
    ...
    static {
        ...
        if (StrUtil.isNotBlank(defaultOssDir)) {
            // 检查必须'/'结束
            defaultOssDir = defaultOssDir.endsWith(AeyeFileReadWriteUtil.DIR_SPLIT_MARK) ? defaultOssDir
                    : defaultOssDir + AeyeFileReadWriteUtil.DIR_SPLIT_MARK;
        } else {
    ...
    public static AeyeFSEntity putObject(String bucket, AeyeFSEntity fsEntity) throws Exception {
        ...
        try {
            ...
            if (!checkFormats(fsEntity.getName())) {
                throw new BusinessException(HSAFExceptionCode.HDF_MODULE_OSS_05.getCode(),
                        HSAFExceptionCode.HDF_MODULE_OSS_05.getMsg() + "，文件名：" + fsEntity.getName());
            }

            if (fsEntity.getInputstream() == null) {
                throw new AppException(HSAFExceptionCode.HDF_MODULE_OSS_03.getCode(),
                        HSAFExceptionCode.HDF_MODULE_OSS_03.getMsg());
            }
            // 长度校验
            String fileName = originalFileName(fsEntity.getName());
            if (StrUtil.isNotBlank(fileName) && fileName.length() > maxFileNameLength) {
                throw new AppException(HSAFExceptionCode.HDF_MODULE_OSS_04.getCode(),
                        HSAFExceptionCode.HDF_MODULE_OSS_04.getMsg() + "【" + maxFileNameLength + "】，文件名："
                                + fsEntity.getName());
            }
            ...
            if (fsEntity.getName().contains(DATETIME_MONTH_KEY)) {
                fsEntity.setName(
                        fsEntity.getName().replace(DATETIME_MONTH_KEY, AeyeJdk8DateUtil.MONTH.format(nowTime)));
            }
            ...
            // 关闭流
            if (null != fsEntity) {
                IoUtil.close(fsEntity.getInputstream());
            }
        }
        return fsEntity;
    }
    ...
    public static boolean exist(String bucket, String key) throws Exception {
        ...
        try {
            ...
            if (entity != null) {
                IoUtil.close(entity.getInputstream());
            }
        }
    }
    ...
    public static boolean deleteObject(String bucket, String keyId) {
        ...
        FSManager fsManager = AeyeSpringContextUtils.getBean(FSManager.class);
        return fsManager.deleteObject(StrUtil.isNotBlank(bucket) ? bucket : defaultBucket,
                new String(Base64Utils.urlDecode(keyId)));
    }
    ...
    public static AeyeFSEntity getObjectEntity(String bucket, String keyId, boolean isLoadData) {
        ...
        try {
            if (FSSTORE_TYPE_FDFS.equals(fsstoreType)) {
                ...
                fsEntity.setInputstream(AeyeFileReadWriteUtil.readFileStream(
                        new URL(fssStoreUrl + AeyeFileReadWriteUtil.DIR_SPLIT_MARK + fsEntity.getName())));
            } else {
                FSManager fsManager = AeyeSpringContextUtils.getBean(FSManager.class);

                fsEntity = fsManager.getObject(StrUtil.isNotBlank(bucket) ? bucket : defaultBucket,
                        new String(Base64Utils.urlDecode(keyId)));
            }
    ...
    public static byte[] packZipFiles(String zipFileName, Map<String, String> fileMapping) throws Exception {
        if (fileMapping.size() > maxZipLength) {
            throw new BusinessException(HSAFExceptionCode.HDF_MODULE_OSS_02.getCode(),
                    HSAFExceptionCode.HDF_MODULE_OSS_02.getMsg() + "，当前=" + fileMapping.size() + "；最大限制="
                            + maxZipLength);
        }
            ...
            for (String keyId : fileMapping.keySet()) {
                ...
                if (data != null) {
                    ...
                    if (zipSize > maxZipSize) {
                        throw new BusinessException(HSAFExceptionCode.HDF_MODULE_OSS_02.getCode(),
                                HSAFExceptionCode.HDF_MODULE_OSS_02.getMsg() + "，当前=" + zipSize + "；最大限制="
                                        + maxZipSize);
                    }
    ...
    private static void buildLogMsg(String msg, StringBuilder buffer) {
        buffer.append(AeyeJdk8DateUtil.DATETIME_MINUTE_CN
                .format(AeyeJdk8DateUtil.convertToLocalDateTime(new Date(System.currentTimeMillis())))).append(":")
                .append(msg).append("\r\n");
    }
}
