...
public class EasyGmUtils {
    ...
    private static int KEY_LENGTH_64 = 64;
    private static int KEY_LENGTH_65 = 65;
    private static int KEY_LENGTH_2 = 2;
    ...
    public static boolean verifySm3WithSm2(byte[] msg, byte[] userId, byte[] rs, byte[] publicKeyBytes) {
        if (publicKeyBytes.length != KEY_LENGTH_64 && publicKeyBytes.length != KEY_LENGTH_65){
            throw new RuntimeException("err key length");
        }

        BigInteger x, y;
        if (publicKeyBytes.length > KEY_LENGTH_64) {
    ...
    private static byte[] rsPlainByteArrayToAsn1(byte[] sign) {
        if (sign.length != RS_LEN * KEY_LENGTH_2) {
    ...
    public static GenKeyDto createGmKey() throws Exception{
        KeyPairGenerator g = KeyPairGeneratorSpi.getInstance("EC", BcUtils.getBcProvider());
        g.initialize(EasyGmUtils.ecParameterSpec,new SecureRandom());
        KeyPair pair = g.generateKeyPair();
        byte[] pubkeyX = ((BCECPublicKey)pair.getPublic()).getQ().getXCoord().getEncoded();
        byte[] pubkeyY = ((BCECPublicKey)pair.getPublic()).getQ().getYCoord().getEncoded();

        byte[] pubkey = new byte[pubkeyX.length+pubkeyY.length];
        System.arraycopy(pubkeyX, 0, pubkey, 0, pubkeyX.length);
        System.arraycopy(pubkeyY, 0, pubkey, pubkeyX.length, pubkeyY.length);

        GenKeyDto keyDto = new GenKeyDto();
        keyDto.setPrivateKey(((BCECPrivateKey)pair.getPrivate()).getD().toByteArray());
        keyDto.setPublicKey(pubkey);
        return keyDto;
    }
    ...
    public static byte[] sm4Encrypt(byte[] keyBytes, byte[] plain) {
        if (keyBytes.length != KEY_LENGTH_16) {
            throw new RuntimeException("err key length");
        }
...
//        if (keyBytes.length != 16) throw new RuntimeException("err key length");
        if (cipher.length % KEY_LENGTH_16 != 0) {
